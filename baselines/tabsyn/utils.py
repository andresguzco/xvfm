...
def execute_function(method, mode):
    ...
    if method == "vae":
        ...
        module_name = (
            f"baselines.tabddpm.main_train"
            if mode == "main"
            else f"baselines.tabddpm.main_sample"
        )
    else:
...
def get_args():
    ...
    parser.add_argument(
        "-e", "--epochs", default=1000, type=int, help="Number of training epochs"
    )
    parser.add_argument(
        "--no-header",
        dest="header",
    ...
    # General Options
    parser.add_argument(
        "--logdir", type=str, default="./codi_exp", help="log directory"
    )
    ...
    parser.add_argument(
        "--encoder_dim_con",
        type=str,
        default="512,1024,1024,512",
        help="encoder_dim_con",
    )
    parser.add_argument(
        "--encoder_dim_dis",
        type=str,
        default="512,1024,1024,512",
        help="encoder_dim_dis",
    )
    ...
    # Training
    parser.add_argument(
        "--training_batch_size", type=int, default=4096, help="batch size"
    )
    ...
    parser.add_argument(
        "--beta_1", type=float, default=0.00001, help="start beta value"
    )
    parser.add_argument("--beta_T", type=float, default=0.02, help="end beta value")
    parser.add_argument(
        "--lr_con", type=float, default=2e-03, help="target learning rate"
    )
    parser.add_argument(
        "--lr_dis", type=float, default=2e-03, help="target learning rate"
    )
    parser.add_argument(
        "--total_epochs_both", type=int, default=20000, help="total training steps"
    )
    parser.add_argument(
        "--grad_clip", type=float, default=1.0, help="gradient norm clipping"
    )
    parser.add_argument("--parallel", action="store_true", help="multi gpu training")

    # Sampling
    parser.add_argument(
        "--sample_step", type=int, default=2000, help="frequency of sampling"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="fixedsmall",
        choices=["fixedlarge", "fixedsmall"],
        help="variance type",
    )

    # Contrastive Learning
    parser.add_argument(
        "--ns_method", type=int, default=0, help="negative condition method"
    )
