...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        alphas_cumprod_next = torch.tensor(
            np.append(alphas_cumprod[1:], 0.0)
        )  # tilde_alpha2_{t+1}
    ...
    def loss_at_step_t(self, x, step):
        ...
        recon_x0_num = self.recon_x0(x_in, model_out, t)[
    ...
    def recon_x0(self, x, model_out, t):
        ...
        x0 = extract(self.sqrt_recip_alphas_cumprod, t, x.shape) * (
            x - model_out * extract(self.sqrt_one_minus_alphas_cumprod, t, x.shape)
        )
    ...
    def mixed_elbo(self, x0):
        ...
        for t in range(self.num_timesteps):
            ...
            model_out = self._denoise_fn(
                torch.cat([x_num_t, log_x_cat_t], dim=1), t_array
            )
    ...
    @torch.no_grad()
    def gaussian_ddim_reverse_sample(self, x, T):
        b = x.shape[0]
        device = x.device
        for t in range(T):
    ...
    @torch.no_grad()
    def multinomial_ddim_step(self, model_out_cat, log_x_t, t, t_prev, eta=1.0):
    ...
    def sample_ddim(self, num_samples, steps=1000):
        ...
        for i in range(0, len(timesteps)):
            ...
            if i != len(timesteps) - 1:
                t_prev = torch.full(
                    (b,), timesteps[i + 1], device=device, dtype=torch.long
                )
            else:
                t_prev = torch.full((b,), 0, device=device, dtype=torch.long)

            model_out = self._denoise_fn(torch.cat([z_norm, log_z], dim=1).float(), t)
            model_out_num = model_out[:, : self.num_numerical_features]
            model_out_cat = model_out[:, self.num_numerical_features :]
            z_norm = self.gaussian_ddim_step(
                model_out_num, z_norm, t, t_prev, clip_denoised=False
            )
            if has_cat:
