...
def get_args():
    ...
    parser.add_argument(
        "--dataset", default=None, type=str, help="Dataset to train the model on"
    )
    parser.add_argument(
        "--data_path", default=None, type=str, help="Path to tabular dataset"
    )
    parser.add_argument(
        "--task_type", default=None, type=str, help="Downstream task to evaluate"
    )
    parser.add_argument(
        "--epochs", default=8000, type=int, help="Number of training epochs"
    )
    parser.add_argument(
        "--batch_size", default=4096, type=int, help="Training batch size"
    )
    parser.add_argument(
        "--lr", default=1e-3, type=float, help="Learning rate for optimizer"
    )
    parser.add_argument(
        "--logging", default=100, type=int, help="Logging interval for training"
    )
    parser.add_argument(
        "--loss", default="llk", type=str, help="Loss function to use for training"
    )
    parser.add_argument(
        "--num_eval", default=1000, type=int, help="Number of synthetic observations"
    )
...
def main(args):
    ...
    dataset = make_dataset(
        args.data_path, num_classes=2 if args.task_type == "regression" else 0
    )
    dataloader = prepare_fast_dataloader(
        dataset, split="train", batch_size=args.batch_size
    )
