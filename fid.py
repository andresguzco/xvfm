...
def get_data_loaders(train_batch_size, val_batch_size):
    ...
    train_loader = DataLoader(
        MNIST(download=True, root="data", transform=data_transform, train=True), 
...
def run(train_batch_size, val_batch_size, epochs, lr, momentum, log_interval, checkdir):
    ...
    pbar.close()
    torch.save( model.state_dict(), f"{checkdir}/fid_model.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_interval", type=int, default=235, help="batches before logging training status")
    parser.add_argument("--checkdir", type=str, default="checkpoints", help="random seed")
    args = parser.parse_args()

    run(args.batch_size, args.val_batch_size, args.epochs, args.lr, args.momentum, args.log_interval, args.checkdir)